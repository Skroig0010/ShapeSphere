package shapesphere.content.contentreaders;
import shapesphere.graphics.*;
import shapesphere.graphics.materials.*;
import shapesphere.graphics.vertices.VertexPositionNormalTexture;
import shapesphere.graphics.vertices.VertexPositionNormalTextureWeights;
import shapesphere.parser.mqo.*;
import shapesphere.parser.x.*;
...
class ModelReader{
    ...
    public function makeModelFromMqo(mqo : Mqo) : Model{
        ...
        var meshes = new Array<shapesphere.graphics.Mesh>();
    ...
    public function makeModelFromX(x : X) : Model{
        ...
        var meshes : Array<shapesphere.graphics.Mesh> = makeMeshesFromXMeshes(xMeshes, bones);
        return new Model(gd, meshes, bones);
    }

    function makeMeshesFromXMeshes(xMeshes : Array<shapesphere.parser.x.Mesh>, bones : Array<Bone>) : Array<shapesphere.graphics.Mesh>{
        var meshes = new Array<shapesphere.graphics.Mesh>();
    ...
    function makeXMeshes(frame : Frame) : Array<shapesphere.parser.x.Mesh>{
        var xMeshes = new Array<shapesphere.parser.x.Mesh>();
    ...
    function makeMeshFromXMesh(xMesh : shapesphere.parser.x.Mesh) : shapesphere.graphics.Mesh{
        // 今回頂点を増やさないで良い
        // Materialが複数ある場合はパーツを分ける
        var vertices = new Array<VertexPositionNormalTexture>();
        var mesh : shapesphere.graphics.Mesh;
        ...
        mesh = new shapesphere.graphics.Mesh(gd, xMesh.name, vertices.flatten().flatten().array(), indices, VertexPositionNormalTexture.vertexDeclaration);
    ...
    function makeMeshFromXMeshWithSkinWeight(xMesh : shapesphere.parser.x.Mesh,skinWeightVertices : Array<{indices : Array<Float>, weights : Array<Float>}>,  bones : Array<Bone>) : shapesphere.graphics.Mesh{
        // 今回頂点を増やさないで良い
        // Materialが複数ある場合はパーツを分ける
        ...
        mesh = new shapesphere.graphics.Mesh(gd, xMesh.name, vertices.flatten().flatten().array(), indices, VertexPositionNormalTextureWeights.vertexDeclaration);
        // parentBoneの設定
        for(bone in bones){
    ...
    function makeSkinnedMaterialFromXMaterial(xMat : Material, bones:Array<Bone>) : IMaterial{
        return new shapesphere.graphics.materials.SkinnedMaterial(gd, xMat.name, gd.shaderProgramCache.getProgram("skinned", "skinned"),
                xMat.color, xMat.power, xMat.spc, xMat.emi, xMat.tex, bones);
    }

    function makeMaterialFromXMaterial(xMat : Material) : IMaterial{
        return new shapesphere.graphics.materials.XMaterial(gd, xMat.name, gd.shaderProgramCache.getProgram("classic", "classic"),
                xMat.color, xMat.power, xMat.spc, xMat.emi, xMat.tex);
    }

    function makeMeshFromMqoObject(object : Object, mqo : Mqo) : shapesphere.graphics.Mesh{
        var vertices = new Array<VertexPositionNormalTexture>();
        var mesh : shapesphere.graphics.Mesh;
        ...
        mesh = new shapesphere.graphics.Mesh(gd, object.name, vertices.flatten().flatten().array(), indices, VertexPositionNormalTexture.vertexDeclaration);
        for(part in meshParts){
            part.setParent(mesh);
        }
        return mesh;
